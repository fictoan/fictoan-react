...
const normalizeClassNames = (forPlainHTML: any, forReact: any, otherClassNames: any) => {
    return [ forPlainHTML, forReact, ...otherClassNames ].filter(Boolean).join(" ");
};

export const Element = React.forwardRef(
    <K extends {}>(props: ElementProps<K> & { class?: string }, ref: React.LegacyRef<HTMLElement>) => {
        const {
                  as        : Component,
                  class     : forPlainHTML,
                  className : forReact,
        ...
        const combinedClassName = normalizeClassNames(forPlainHTML, forReact, classNames);
    ...
    }) as <K extends {}>(props: ElementProps<K> & {
    ref?: React.LegacyRef<HTMLElement>, class?: string
}) => React.ReactElement;
